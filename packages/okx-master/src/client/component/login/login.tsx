    ...
    >
      <LoginFormPage
        onFinish={async (data: { username: string; password: string }) => {
          ...
          const result = await fetch('/api/login/password', {
            method: 'POST',
            headers: {
          ...
          const json = (await result.json()) as Record<string, string>;
          if (json.err) {
            void message.error(json.msg);
          } else {
            navigate('/');
          }
          return true;
        ...
        activityConfig={{
          ...
          title: 'Promotion',
          subTitle: 'Just for you, just in this month',
          action: (
            <Button
              size="large"
              style={{
      ...
      >
        ...
        >
          <Tabs.TabPane key={'account'} tab={'User Account'} />
          <Tabs.TabPane key={'phone'} tab={'Mobile Number'} />
        </Tabs>
        {loginType === 'account' && (
          <>
            <ProFormText
